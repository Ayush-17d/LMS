...
const ScrollingText: FC<{
  ...
  return (
    ...
    >
      ...
      >
        <span 
          className="select-none whitespace-nowrap font-black uppercase leading-[0.75] text-slate-400 opacity-30 text-[15vmax] md:text-[12vmax] lg:text-[10vmax] xl:text-[15vmax]"
        >
          {fullText}
        </span>
      </motion.div>
...
const Hero: FC<Props> = () => {
  ...
  return (
    <div className="w-full h-screen flex flex-col lg:flex-row items-center justify-between bg-[#DDE6ED] dark:bg-[#27374D] relative overflow-hidden">
      {/* Scrolling Text Layers with responsive positioning */}
      <ScrollingText text="grow daily" direction="left" top={0} />
      <ScrollingText 
        text="Stay Focused" 
        direction="right" 
        top={typeof window !== 'undefined' && window.innerWidth <= 800 ? 100 : 150} 
      />
      <ScrollingText 
        text="Master Skills" 
        direction="left" 
        top={typeof window !== 'undefined' && window.innerWidth <= 800 ? 200 : 300} 
      />
      <ScrollingText 
        text="Unlock Potential" 
        direction="right" 
        top={typeof window !== 'undefined' && window.innerWidth <= 800 ? 300 : 450} 
      />

      {/* Left Side - Image */}
      <div className="image-container z-10 w-full lg:w-1/2 max-w-[500px] p-4">
        <Image
          src={Image1}
          alt="Hero Image"
          className="object-cover select-none w-full h-full rounded-full"
      ...
      <div className="w-full lg:w-1/2 flex flex-col items-center lg:items-start justify-center px-6 lg:px-12 text-center lg:text-left space-y-8 z-10">
        {/* Heading Section */}
        <h1 className="text-3xl select-none md:text-4xl lg:text-5xl font-thin text-gray-800 dark:text-white relative">
          Welcome to <span className="relative">LearnifyHub</span>
        </h1>

        {/* Subheading */}
        <p className="text-base md:text-lg lg:text-xl text-gray-600 dark:text-white">
          Elevate your skills, master your goals - start learning now.
        </p>
        ...
        <form className="flex w-full items-center max-w-lg" onSubmit={handleSearch}>
          ...
          <div className="relative w-full">
            ...
            <input
              ...
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 md:w-[400px]"
              placeholder="Search Courses..."
            />
          </div>
          <button
            type="submit"
            className="ml-2 p-2.5 text-sm font-medium text-white bg-blue-700 rounded-lg border border-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"
          >
            <FaSearch className="w-4 h-4" />
            <span className="sr-only">Search</span>
          </button>
        ...
        {/* Trust Statement */}
        <div className="text-center lg:text-left">
          <h5 className="text-base md:text-lg text-gray-700 dark:text-gray-300">
            500k+ people already trust us.{" "}
            <a
              href="/courses"
              className="text-blue-600 underline hover:text-blue-800 transition-colors"
